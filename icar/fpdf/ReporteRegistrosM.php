...
class PDF extends FPDF
{
   ...
   function Header()
   {
      ...
      $this->Cell(100, 10, utf8_decode("REPORTE DE MECANICOS REGISTRADOS "), 0, 1, 'C', 0);
      $this->Ln(7);
...
$consulta_reporte_registros = $conn->query("SELECT cedulaM, nombreM, telefonoM FROM mecanicos");
...
if ($consulta_reporte_registros) {
   ...
   if ($consulta_reporte_registros->num_rows > 0) {
       ...
       while ($row = $consulta_reporte_registros->fetch_assoc()) {
           ...
           $cedulaM = $row['cedulaM'];
           $nombreM = $row['nombreM'];
           $telefonoM = $row['telefonoM'];

           $pdf->Cell(90, 10, utf8_decode("$cedulaM"), 1, 0, 'C', 0);
            $pdf->Cell(90, 10, utf8_decode("$nombreM"), 1, 0, 'C', 0);
            $pdf->Cell(95, 10, utf8_decode("$telefonoM"), 1, 1, 'C', 0);
       }
   } else {
